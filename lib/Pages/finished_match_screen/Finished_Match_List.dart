...
import 'Finished_Match_Score.dart';

class FinishedMatchList extends StatefulWidget {
  const FinishedMatchList({Key? key, required this.list, required this.isLoading}) : super(key: key);

  final RxList<AllMatchData> list;
  final bool isLoading;

  @override
  State<FinishedMatchList> createState() => _FinishedMatchListState();
}

class _FinishedMatchListState extends State<FinishedMatchList> {
                            ...
                            onTap: () async {
                              ...
                              AdsHelper.showInterstitialAds(adCloseEvent: () {
                                ...
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) {
                                      return FinishedMatchScore(
                                        title: widget.list[index].title,
                                        matchData: widget.list[index],
                                      );
                                    },
                                  )..popped.then((value) {
                                      // SocketHelper.teamBPlayerList.clear();
                                      // SocketHelper.teamAPlayerList.clear();
                                      // print(SocketHelper.teamAPlayerList.length);
                                      // print(SocketHelper.teamBPlayerList.length);
                                      // SocketHelper.isAllPlayerLoading.value = true;
                                      print("Back-----------------");
                                    }),
                                );
                              });
