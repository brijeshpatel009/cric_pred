...
class _TabContainerState extends AnimatedWidgetBaseState<TabContainer> {
  ...
  TextStyle _textStyle(int i, double progress, int currentIndex, int prevIndex, TextStyle selectedTextStyle, TextStyle unselectedTextStyle) {
    ...
    final double pct = progress == ceil ? 1 : ((progress - floor) / (floor == ceil ? 1 : ceil - floor).abs());
  ...
  double _textScale(int i, double progress, int currentIndex, int prevIndex, TextStyle selectedTextStyle, TextStyle unselectedTextStyle) {
    final int ceil = max(currentIndex, prevIndex);
    final int floor = min(currentIndex, prevIndex);
    final double pct = progress == ceil ? 1 : ((progress - floor) / (floor == ceil ? 1 : ceil - floor).abs());

    if (i == currentIndex) {
      return lerpDouble(1, selectedTextStyle.fontSize! / unselectedTextStyle.fontSize!, prevIndex > currentIndex ? 1 - pct : pct)!;
    } else if (i == prevIndex) {
      return lerpDouble(1, selectedTextStyle.fontSize! / unselectedTextStyle.fontSize!, prevIndex > currentIndex ? pct : 1 - pct)!;
    } else {
      return 1;
    }
  }
  ...
  List<Semantics> _getTabs() {
    ...
    for (int i = 0; i < count; i++) {
      final double scale = _textScale(i, progress?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ?? 0.0, _currentIndex,
          _prevIndex, _selectedTextStyle, _unselectedTextStyle);

      final TextStyle style = _textStyle(i, progress?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ?? 0.0, _currentIndex,
              _prevIndex, _selectedTextStyle, _unselectedTextStyle)
          ...
          child: Transform(
            ...
            child: Container(
              child: !widget.isStringTabs
                  ? widget.tabs[i]
                  : Text(widget.tabs[i], textAlign: TextAlign.center, overflow: TextOverflow.fade, textDirection: _textDirection, style: style),
            ),
          ),
        ),
      );
  ...
  @override
  void initState() {
    _controller = widget.controller ?? TabContainerController(length: widget.tabs.length);
  ...
  Widget build(BuildContext context) {
    return TabFrame(
      ...
      progress: progress?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ?? 0.0,
      ...
      color: phase?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ?? widget.color ?? Colors.transparent,
      enabled: widget.enabled,
...
class RenderTabFrame extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, TabFrameParentData>, RenderBoxContainerDefaultsMixin<RenderBox, TabFrameParentData> {
  ...
  void attach(covariant PipelineOwner owner) {
    ...
    _tapRecognizer = TapGestureRecognizer(debugOwner: this)..onTapDown = !_enabled ? null : _onTapDown;
  }
  ...
  void performLayout() {
    ...
    BoxConstraints mainChildConstraints = BoxConstraints(maxWidth: size.width, maxHeight: size.height - _tabExtent);
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
      mainChildConstraints = BoxConstraints(maxWidth: size.width - _tabExtent, maxHeight: size.height);
    }

    child.layout(mainChildConstraints, parentUsesSize: true);

    final double horizontalGap = (mainChildConstraints.maxWidth - child.size.width) / 2;
    final double verticalGap = (mainChildConstraints.maxHeight - child.size.height) / 2;
    ...
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
      ...
      textConstraints = BoxConstraints(maxWidth: _tabExtent, maxHeight: tabBreadth);
    } else {
      tabBreadth = _tabRange(size.width) / _tabs.length;
      textConstraints = BoxConstraints(maxWidth: tabBreadth, maxHeight: _tabExtent);
    }
    ...
    for (var i = 0; child != null; i++, child = childAfter(child)) {
      ...
      final double horizontalGap = (textConstraints.maxWidth - child.size.width) / 2;
      final double verticalGap = (textConstraints.maxHeight - child.size.height) / 2;

      switch (_tabEdge) {
        case TabEdge.left:
          textOffset = Offset(horizontalGap, verticalGap + indexOffset + _tabStartPosition(size.height));
          break;
        case TabEdge.top:
          textOffset = Offset(horizontalGap + indexOffset + _tabStartPosition(size.width), verticalGap);
          break;
        case TabEdge.right:
          textOffset = Offset(size.width - horizontalGap - child.size.width, verticalGap + indexOffset + _tabStartPosition(size.height));
          break;
        case TabEdge.bottom:
          textOffset = Offset(horizontalGap + indexOffset + _tabStartPosition(size.width), size.height - verticalGap - child.size.height);
          break;
      }
  ...
  double computeMinIntrinsicWidth(double height) {
    ...
    final altMinIntrinsicWidth = _radius * 2 * _tabs.length / (_tabEnd - _tabStart);
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
  ...
  double computeMaxIntrinsicWidth(double height) {
    ...
    final altMaxIntrinsicWidth = _radius * 2 * _tabs.length / (_tabEnd - _tabStart);
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
  ...
  double computeMinIntrinsicHeight(double width) {
    ...
    final altMinIntrinsicHeight = _radius * 2 * _tabs.length / (_tabEnd - _tabStart);
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
  ...
  double computeMaxIntrinsicHeight(double width) {
    ...
    final altMaxIntrinsicHeight = _radius * 2 * _tabs.length / (_tabEnd - _tabStart);
    if (_tabEdge == TabEdge.left || _tabEdge == TabEdge.right) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final Path horizontalPath = Path()
      ...
      ..lineTo(min(max(width - _radius, rightPos), min(width, rightPos + _radius)), height - _tabExtent)
      ..quadraticBezierTo(
        rightPos,
        height - _tabExtent,
        rightPos,
    ...
    final Path verticalPath = Path()
      ...
      ..lineTo(_tabExtent, min(max(height - _radius, rightPos), min(height, rightPos + _radius)))
      ..quadraticBezierTo(
        _tabExtent,
        rightPos,
        _tabExtent - _radius,
